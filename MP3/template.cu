...
__global__ void matrixMultiplyShared(float *A, float *B, float *C,
  ...
  for(int i = 0;i<(numBRows-1)/TILE_WIDTH+1;++i){
    if((Row<numARows) && ((i*TILE_WIDTH+tx)<numAColumns)){
      subA[ty][tx] = A[Row * numAColumns+i *TILE_WIDTH+tx];
    }
    else{
      subA[ty][tx] = 0.0;
    }
    if((Col<numBColumns) && ((i*TILE_WIDTH+ty)<numBRows)){
      subB[ty][tx] = B[(i*TILE_WIDTH+ty)*numBColumns+Col];
    }
    else{
      subB[ty][tx] = 0.0;
    }
    ...
    for(int k=0;k<TILE_WIDTH ;++k){
      cur+=subA[ty][k]*subB[k][tx];
    }
    __syncthreads();

    if((Row<numCRows) && (Col<numCColumns)){
      C[Row*numCColumns+Col] = cur;
    } 
  }
